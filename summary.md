...
Представим обучающую платформу, на которой люди выполняют задания и получают баллы (как степик). В базе данных такой платформы может быть таблица `activity`, которая хранит баллы, набранные пользователями в конкретные дни:
...
Таблица игрушечная, поэтому сначала посчитаем руками:
...
К сожалению, каждая СУБД реализует работу с датами по-своему, поэтому универсального решения не существует. В MySQL есть функция `to_days()`:
...
```sql
with agroups as (select adate,
                        unixepoch(adate) / 86400 - dense_rank() over w as group_id
                 from activity
                 where user_id = 51
                 window w as (order by adate))
select min(adate) as day_start,
...
Мы начали урок с того, что при анализе данных бывает удобно разбить датасет на группы близких значений. Но «близкие» не всегда означает «идущие подряд». Иногда мы готовы включить в одну группу значения с небольшими промежутками.
...
Договоримся, что соседние значения входят в одну и ту же группу, если отличаются на 1 или 2. Тогда получится три группы (или кластера):

- (51, 52, 53, 55, 56)
- (59, 61)
- (72)

Попробуем найти кластеры с помощью SQL. Подход с ранжированием строк здесь не сработает, поскольку мы разрешили промежутки между соседними значениями. Поэтому поступим иначе.
...
Границы уже явно видны, пронумеруем их по порядку:
...
Договоримся, что пропуск одного дня не прерывает серию. Как тогда будут выглядет кластеры?
...
Посчитаем кластеры с помощью SQL. Вспомним универсальный алгорим:
...
select adate,
       ...
       lag(unixepoch(adate) / 86400) over w as day_lag
from activity
where user_id = 21
window w as (order by adate);
```
...
with adays as (select adate,
               ...
               window w as (order by adate)),
-- границы кластеров
     agroups as (select adate,
                        day_lag,
                        sum(case when day_lag > 2 then 1 else 0 end)
                            over w as group_id
                 from adays
                 window w as (order by adate))
-- сами кластеры
...
Если вы работаете с данными из надежного источника — например, внутренней ИТ-системы компании — по ним обычно можно сразу строить аналитические отчеты. Но если загрузили датасет из просторов интернета или получили от внешнего партнера — не факт, что там все хорошо. Такие данные приходится предварительно чистить. Оконные функции могут с этим помочь.
...
Будем работать с таблицей `weather` - погодой в Лондоне за 6 месяцев 2020 года.
...
Бывает, что в данных заполнены не все значения. Например, в нашем датасете:
...
За полгода набралось 6 дней без температуры. Можно было бы вовсе удалить их, но мы применим другую технику: заполним пропуски предыдущим значением.

Для наглядности будем работать только с февралем:
...
5 февраля должно получить температуру от предыдущего дня — 6.7 °C.

Для начала, с помощью функции `lag()` вынесем температуру предыдущего дня в отдельный столбец `prev`:

```sql
select wdate,
       wtemp,
       lag(wtemp, 1) over w as prev
from weather
where wdate between '2020-02-01' and '2020-02-08'
...
Затем выберем значение `prev` в тех случаях, когда `wtemp` пустое. В этом поможет стандартная функция `coalesce()`, которая возвращает первое не-null значение:

```sql
select wdate,
       wtemp,
       coalesce(wtemp, lag(wtemp, 1) over w) as fixed
from weather
where wdate between '2020-02-01' and '2020-02-08'
window w as (order by wdate);
```
...
│   wdate    │ wtemp │ fixed │
├────────────┼───────┼───────┤
│ 2020-02-01 │ 11.2  │ 11.2  │
│ 2020-02-02 │ 9.9   │ 9.9   │
│ 2020-02-03 │ 10.3  │ 10.3  │
│ 2020-02-04 │ 6.7   │ 6.7   │
│ 2020-02-05 │       │ 6.7   │
│ 2020-02-06 │ 4.8   │ 4.8   │
│ 2020-02-07 │ 5.0   │ 5.0   │
...
Теперь в столбце `fixed` получилась температура без пропусков.

### Предыдущее непустое значение

Посмотрим на февраль:
...
Здесь подход «заполнить предыдущим значением» тоже сработает. Но что делать, если и предыдущее значение пустое?

```sql
update weather
set wtemp = null
where wdate = '2020-02-04'
```
...
Теперь установить температуру за 5 февраля не получится — ведь значение за 4 февраля тоже пустое.

Можно применить другой подход: брать не просто предыдущее, а предыдущее непустое значение. Тогда и 4, и 5 февраля получат температуру от 3 февраля — 10.3 °C.

Для начала, определим группы дней, как мы это делали на уроке «Кластеризация». Будем считать, что номер группы увеличивается каждый раз, когда мы встречаем день с заполненной температурой:

```sql
select wdate,
       wtemp,
       sum(case when wtemp is not null then 1 else 0 end) over w as group_id
from weather
where wdate between '2020-02-01' and '2020-02-08'
window w as (order by wdate);
```
...
Благодаря такому подходу, проблемные дни 4 и 5 февраля попали в одну группу с «эталонным» 3 февраля. Осталось подставить температуру от 3 февраля с помощью функции `first_value()` и секции по `group_id`:

```sql
-- группы по температуре
with src as (select wdate,
                    wtemp,
                    sum(case when wtemp is not null then 1 else 0 end)
                        over w as group_id
             from weather
             where wdate between '2020-02-01' and '2020-02-08'
             window w as (order by wdate))
-- предыдущее непустое значение температуры
select wdate,
       wtemp,
       first_value(wtemp) over w as fixed
from src
window w as (
...
Часто набор данных удобно характеризовать одним значением, чтобы не закапываться в детали. Например, посчитать среднюю температуру по каждому месяцу:

```sql
select strftime('%Y-%m', wdate) as wmonth,
       round(avg(wtemp), 2)     as avg_temp
from weather
group by
    strftime('%Y-%m', wdate)
order by
...
Как мы обсуждали на уроке «Статистика», со средним значением есть проблема: оно подвержено влиянию выбросов (экстремально больших или малых значений). Поэтому будем использовать медиану.

Медиану можно рассчитать через функцию `percentile_cont()`:

```sql
select to_char(wdate, 'YYYY-MM')                                              as wmonth,
       round(avg(wtemp) ::decimal, 2)                                         as avg_temp,
       round(percentile_cont(0.5) within group (order by wtemp) ::decimal,
             2)                                                               as med_temp
from weather
group by
...
Есть и другой путь. Можно использовать среднее арифметическое, но предварительно отсеять выбросы. Например:

- выкинуть 5% самых маленьких значений (то есть значения меньше 5-го процентиля);
- выкинуть 5% самых больших значений (то есть значения больше 95-го процентиля);
- посчитать среднее арифметическое от оставшихся.

Считаем процентили:

```sql
select to_char(wdate, 'YYYY-MM') as wmonth,
       min(wtemp)                as temp_min,
       percentile_cont(0.05)        within
group
    (order by wtemp) as temp_5,
  percentile_cont(0.95) within
group
    (
    order by wtemp) as temp_95, max(
    wtemp) as temp_max
from weather
...
```text
┌─────────┬──────────┬────────┬─────────┬──────────┐
│ wmonth  │ temp_min │ temp_5 │ temp_95 │ temp_max │
├─────────┼──────────┼────────┼─────────┼──────────┤
│ 2020-01 │ 3.0      │ 3.92   │ 11.06   │ 11.8     │
│ 2020-02 │ 4.8      │ 5.04   │ 11.88   │ 12.5     │
│ 2020-03 │ 4.2      │ 5.74   │ 11.81   │ 12.5     │
│ 2020-04 │ 4.2      │ 8.39   │ 16.79   │ 17.2     │
│ 2020-05 │ 8.6      │ 9.08   │ 20.16   │ 20.5     │
│ 2020-06 │ 10.9     │ 13.44  │ 25.02   │ 25.9     │
└─────────┴──────────┴────────┴─────────┴──────────┘
```

Считаем среднее по температурам, попадающим в интервал от 5 до 95 процентилей:

```sql
-- границы
with avgs as (select to_char(wdate, 'YYYY-MM') as wmonth,
                     percentile_cont(0.05) within group (order by wtemp) as p5,
                     percentile_cont(0.95) within group (order by wtemp) as p95
              from weather
              group by to_char(wdate, 'YYYY-MM'))
-- средние значения
select to_char(wdate, 'YYYY-MM')      as wmonth,
       round(avg(wtemp) ::decimal, 2) as temp_avg,
       round(avg(wtemp) filter (where wtemp between avgs.p5 and avgs.p95)
             ::decimal, 2)            as adjusted
from weather
         join avgs on to_char(wdate, 'YYYY-MM') = avgs.wmonth
group by
    to_char(wdate, 'YYYY-MM')
order by
    wmonth;
